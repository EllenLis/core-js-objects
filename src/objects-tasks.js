...
function sellTickets(queue) {
  ...
  for (let i = 0; i < queue.length; i += 1) {
    if (queue[i] === 25) {
      ...
      if (billFirty >= 1 && billTwFive >= 1) {
        billFirty -= 1;
        billTwFive -= 1;
      } else if (billTwFive >= 3) {
        billTwFive -= 3;
      } else {
        return false;
      }
...
const cssSelectorBuilder = {
  selector: '',
  uniqueParts: ['element', 'id', 'pseudoElement'],
  order: ['element', 'id', 'class', 'attr', 'pseudoClass', 'pseudoElement'],
  orderTracker: [],

  validateUniqueness(type) {
    if (this.uniqueParts.includes(type) && this.orderTracker.includes(type)) {
      throw new Error(
  ...
  validateOrder(type) {
    const lastIndex =
      this.orderTracker.length > 0
        ? this.order.indexOf(this.orderTracker[this.orderTracker.length - 1])
        : -1;
    ...
    if (currentIndex === lastIndex && this.uniqueParts.includes(type)) {
      throw new Error(
        'Element, id and pseudo-element should not occur more then one time inside the selector'
      );
    }
  ...
  add(type, value) {
    ...
    newBuilder.orderTracker = this.orderTracker.concat(type);
